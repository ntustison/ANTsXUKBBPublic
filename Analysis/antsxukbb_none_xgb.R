                                         ...
                                         merge( idpData$ants_dkt_region_volumes$Data,
                                           merge( idpData$ants_deep_flash$Data,
                                             merge( idpData$ants_deep_atropos$Data,
                                              merge( idpData$ants_cerebellum$Data,
...
for( i in seq.int( length( modelData ) ) )
  {
  cat( "Writing ", modelNames[i], "\n" )
  write.csv( modelData[[i]], paste0( dataDirectory, "antsxukbb_deeplearning_", modelNames[i], ".csv" ), row.names = FALSE, quote = FALSE )
  }
write.csv( targetMeasures, paste0( dataDirectory, "antsxukbb_deeplearning_targets.csv" ), row.names = FALSE, quote = FALSE )
...
cat( "Target: ", colnames( targetMeasures )[i], "\n" )
...
#   {
  ...
  measureFilename <- paste0( dataDirectory, "ExperimentalResults/Xgboost90PercentTrainingData/ukbiobank_none_xgboost_", targetName, "_prediction.csv" )
  ...
  antsxukbbXgboost( brainData, response = targetName, type = targetMeasuresType[i],
    ...
    plotFilename = paste0( figuresDirectory, "none_xgb_", targetName ),
    plotWidth = 3, plotHeight = 3 )

  cat( "![", targetName, "]", "(", normalizePath( paste0( figuresDirectory, "none_xgb_", targetName, ".pdf" ) ), ")", sep = "" )
  if( targetMeasuresType[i] == "regression" )
    {
    cat( "![", targetName, "]", "(", normalizePath( paste0( figuresDirectory, "none_xgb_", targetName, "RegressionRegion.pdf" ) ), ")", sep = "" )
    }

  # knitr::include_graphics( c( paste0( figuresDirectory, "none_xgb_", targetName, ".pdf" ),
  #                             paste0( figuresDirectory, "none_xgb_", targetName, "RegressionRegion.pdf" ) ) )
