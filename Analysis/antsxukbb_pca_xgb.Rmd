...
```{r xgb-pca, message = FALSE, results = 'asis', echo = FALSE, eval = TRUE}
...
for( i in seq.int( length( modelData ) ) )
  {
  cat( "Doing model", i, "\n" )
  # s <- spca( scale( as.matrix( modelData[[i]][trainingIndices,] ) ), 50,
  #            para = rep( 100, 50 ), sparse = "varnum", trace = TRUE )
  s <- prcomp( as.matrix( modelData[[i]][trainingIndices,] ), retx = FALSE,
               center = TRUE, scale. = TRUE, tol = NULL, rank. = numberOfFeatures )
  modelPcaData[[i]] <- scale( modelData[[i]][-trainingIndices,], center = TRUE, scale = TRUE ) %*% s$rotation
  }
...
for( i in seq.int( ncol( targetMeasures ) ) )
  ...
  rmseFilename <- paste0( dataDirectory, "GradWarpExperiments/ukbiobank_pca_xgboost_", targetName, "_prediction.csv" )
  ...
  brainData <- list()
  for( j in seq.int( length( modelPcaData ) ) )
    {
    brainData[[j]] <- cbind( targetMeasures[-trainingIndices, i], modelPcaData[[j]] )
  ...
  for( j in seq.int( length( modelPcaData ) ) )
    {
    brainDescriptions[[j]] <- paste0( modelNames[j], seq.int( numberOfFeatures ) )
    }

  antsxukbbXgboost( brainData, response = targetName, pipelineNames = modelNames,
    ...
    plotFilename = paste0( "Figures/pca_xgb_", targetName ),
    plotWidth = 3, plotHeight = 3 )
  knitr::include_graphics( paste0( "Figures/pca_xgb_", targetName, ".pdf" ) )
  }
