...
if( shuffleIndices )
  {
  set.seed( 1234 )
  shuffledIndices <- sample( 1:ncol( allModelsData ) ) 
  allModelsData <- allModelsData[, shuffledIndices]
  allModelsDescriptions <- allModelsDescriptions[shuffledIndices]

  modelData <- list( allModelsData )
  modelDescriptions <- list( allModelsDescriptions )
  modelNames <- c( "All" )
  } else {
...
cat( "Target: ", colnames( targetMeasures )[i], "\n" )
  ...
  measureFilename <- paste0( dataDirectory, "ExperimentalResults/LinearModel90PercentTrainingData/ukbiobank_none_glmnet_", targetName, "_prediction.csv" )
  ...
  antsxukbbLinearModel( brainData, response = targetName, type = targetMeasuresType[i],
    ...
    plotFilename = paste0( figuresDirectory, "none_glmnet_", targetName ),
    plotWidth = 3, plotHeight = 3, doPenalizedRegression = TRUE,
    glmnetAlpha = 0 )
#   }
